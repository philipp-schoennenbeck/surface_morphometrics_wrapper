...
def export_csv(tg, csvname, verbose=False):
    ...
    for property in scalar_properties:
        ...
        df[property] = np.array(list(tg.graph.vp[property])) #get_vertex_property_array(property)
...
def surface_verticality(graph_file, exportcsv=False, verbose=False):
    ...
    if exportcsv:
        export_csv(tg, graph_file[:-4]+".csv", verbose=verbose)

def surface_self_distances(graph_file, surface_file, dist_min=6, dist_max=400, tolerance=0.1, exportcsv=True, verbose=False):
